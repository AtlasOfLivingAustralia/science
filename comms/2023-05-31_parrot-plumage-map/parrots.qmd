...
The trick to this dataviz is the randomness by which points are plotted. Without randomness, each species' points are plotted at once creating a 'clump' effect. However, plotting randomly creates a 'paint-like' effect as patterns of points are created through the mixtures of colours.

Data downloaded from galah is not random. We've provided an additional dataset to preserve the original random order of the points
...
These parquets are too large to upload to GitHub repo. They are in Teams [here](https://csiroau.sharepoint.com/:f:/r/sites/AtlasofLivingAustraliaTeams/Shared%20Documents/Teams/Science%20and%20Decision%20Support/Data/science/comms/2023-05-31_parrot-plumage-map/data?csf=1&web=1&e=JHfjgk).

```{r}
parquet_parrots <- here("comms", "2023-05-31_parrot-plumage-map", "data", "parrots-data") |> read_parquet()
parquet2_parrots <- here("comms", "2023-05-31_parrot-plumage-map", "data", "parrots-randomly-ordered") |> read_parquet()
```
