...
Mapping the potential range of species is an interesting way to learn about the species itself and what influences where it is found. It can also be vital for conserving the species both now and in the future. 
Here we use a specie distribution model to predict the range and probability of Nudibranch occurrence across Australia. 

------------------------------------------------------------------------

**Species distribution models** (SDM) can be used to predict the range of different organisms and can model how changing environmental conditions impact their occurrence probability. These are particularly useful for management, conservation and predicting the impacts of climate change. Here we will use an SDM to predict the occurrence probability of Nudibranchs around Australia and map this monthly to look at seasonal changes. This post is inspired by Liam Bailey's Bigfoot SDM map and code which can be found [here](https://github.com/LiamDBailey/TidyTuesday/blob/93cbe1143757c97ddaf36c564ac03a2b8088cb56/R/2022/Week37_2022.qmd)


Before we begin, it is important to first understand the basics behind SDMs. SDM are built using different types of statistical models depending on the type and suitability of your data. However in every SDM there are always 2 main inputs: 
...
First install all the packages needed for this post. 
...
Now that those have been installed we will use the galah package and the following code to download the observations of Nudibranchia across Australia:
...
# Clean data to remove duplicate rows 
...
Now that the we have the occurrences formatted correctly it is time to do the same to the environmental variables. For this we need to download a set of variables and then limit the geographical boundaries to just the coast of Australia. 
...
First create the colour palette for our map:
...
Now we have produced a species distribution map of nudibranchs across Australia! 
From this, we can see there is a high probability of seeing them around the coast, and this decreases further away from the coastline towards deeper water. This map provides an excellent overview of nudibranchs across Australia; however, we can zoom into specific regions where we will see a more detailed view of their potential distribution. 

For this we need to limit the map to a specific region within ggplot. First lets download point data of the main cities in Australia so we can visualize the map locations better. 
...
make_months <- function(chosen_month) {
  ...
  monthly_data <- nudibranchia_occurrences %>% filter(month == {{chosen_month}}) %>%
