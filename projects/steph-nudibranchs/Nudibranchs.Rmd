...
ggplot() +
  geom_stars(data = species_distribution) +
geom_sf(data = ozmaps::ozmap_country, colour = "grey", fill = "#C8C6AF")  +
coord_sf(crs = "WGS84", xlim= c(143,154), ylim = c(-37, -11)) +
theme_void() +
...
labs(fill = "Probability") +
 scale_fill_gradientn(colours = c(colour_palette),
                       limits = c(0, 1),
                       guide = guide_colourbar(title = "Probability/Speices Occurrence", 
                                               title.theme = element_text(family = "Times New Roman"),
                                               size = 20,
                                               ticks = FALSE,
                                               title.position = "top"),
...
```{r echo=TRUE}
# Map-making function
plot_the_map <- function(model_data) {
  
ggplot() + 
  geom_stars(data = model_data) +
geom_sf(data = ozmaps::ozmap_country, colour = "grey", fill = "#C8C6AF")  +
coord_sf(crs = "WGS84", xlim= c(140,154), ylim = c(-28.5, -11)) +
theme_void() +
  ...
  theme(panel.border = element_rect(color = "#FFFFFF",
                                   fill = NA,
                                    size = 2)) +
labs(fill = "Probability") +
 scale_fill_gradientn(colours = c("#BBDEF0", "#19647E", "#108D94", "#FFC857", "#D34E24", "#F16704"),
                       limits = c(0, 1),
                       guide = guide_colourbar(title = "Occurrence probability", 
                                               title.theme = element_text(family = "Times New Roman"),
                                               size = 10,
...
number_of_maps <- 1:12 # set number equal to total number
