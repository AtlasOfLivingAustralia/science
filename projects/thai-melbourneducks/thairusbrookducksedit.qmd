...
Allmelbrecords_data$year <- as.numeric(Allmelbrecords_data$year)
...
Ducksbyweek_scaled <- 
  ...
  names_sort = TRUE) |> 
  #mutate(proportyearlypecerentage = as.integer(across(.cols = "2017":"2021", ~ sum())))
  mutate(proportyearlypecerentage = "2017":"2021" / yearlytotal*100)
Ducksbyweek_scaled |> tail(370L)
```

```{r}
left_join(Ducksbyweek_scaled, Allmelbrecords_data, by = c("year", "year"))
```
...
Ducksbyweek_pilot4 <- Ducksbyweek_pilot3 +
  ...
  scale_x_continuous(limits = c(1, 106), 
                     breaks = c(1, 12.5, 25, 37.5, 53, 65.5, 78, 90.5, 103), 
                     labels = c("Jan", "April", "Jul", "Oct", "Jan", "April", "Jul", "Sep", "Jan")) + annotation_custom(text_2020, xmin=25,xmax=25, ymin=-3.7, ymax=-3.7) + 
