# ---------------------------------------------------------------------------- #
# Title: GBIF + ALA citations
# Author: Callum Waite
# Date: Sometime in 2023 or maybe 2024
# ---------------------------------------------------------------------------- #

# Taken from supplementary material in this published paper:
# https://academic.oup.com/bioscience/article/74/9/640/7745614
...
##### Get Datasets #####
dataset_df <- request("https://api.gbif.org/v1/dataset/search/export?
publishingCountry=AU&format=CSV") |>
  req_perform() |>
  resp_body_string() |>
  read_csv()
##### Takes ~2hrs to run #####
...
all_papers_clean <- all_papers |>
  select(-gbifDownloadKey, -gbifOccurrenceKey, -gbifNetworkKey, -gbifProjectIdentifier, -
           gbifProgramme, -tags) |>
  rowwise() |>
  mutate(
...
joined_papers_clean <- joined_papers |>
  filter(!is.na(DOI)) |>
  mutate(true_holder = TRUE) |>
  pivot_wider(names_from = `Citation Source`, values_from = true_holder, values_fill =
                FALSE) |>
...
GBIF_citation_plot <- rbind(
  ...
  scale_fill_manual(name = "GBIF citations that...",
                    values = c("use ALA data" = "#FFA300", "don't use ALA data" =
                                 "#509E2F")) +
  theme_classic() +
...
rbind(
  all_papers_clean |>
    select(authors, added, published, identifiers, keywords, literatureType, openAccess,
           peerReview, source, title, topics, year, abstract, citationType) |>
...
all_citations_totals <- joined_papers_clean |>
  mutate(
    ...
    `Citation Source` = ifelse(`Citation Source` == "ALA", "AU data via ALA", "AU data via
GBIF")) |>
  ...
  mutate(
    ...
    `Citation Source` = factor(`Citation Source`, levels = c("GBIF without AU data", "AU
data via GBIF", "AU data via ALA"))) |>
...
all_citations_plot <- ggplot(all_citations_totals) +
  ...
  theme_bw() +
  ylab("Number of Citations")
ggsave("figures/citation_plot_cw.pdf", all_citations_plot, width = 8, height = 6, units =
         "in")
