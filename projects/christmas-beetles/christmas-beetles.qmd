...
# title
library(marquee)
title_text <- marquee_glue(
  "Christmas beetle observations"
)
subtitle_text <- marquee_glue(
  "{#ab3b36 **Christmas beetles** (*Anoplognathus*)} were once so plentiful in December that people used brooms to sweep them out of their homes. Recently, however, Christmas beetles seem to have mostly disappeared. With relatively few recorded observations of Christmas beetles over the last 100 years, it's difficult to know why or by how much Christmas beetle numbers have dropped. 
  
  The \"Christmas Beetle Count\" initiative (via iNaturalist) is attempting to add data to improve our knowledge about Christmas beetles and their distribution."
)

ggplot(data = c_beetles) +
  ...
  pilot::theme_pilot(axes = "b", grid = "h") + 
  scale_x_continuous(breaks = seq(min(c_beetles$year), 2020, by = 20)) +
  ...
  annotate("text", x = 2007, y = 1800, 
           label = "Christmas beetle\n count starts",
           hjust = 0.5,
           fontface = 'bold', size = 4.5, col = 'grey30', lineheight = 0.9) +
  annotate("curve", x = 2021, xend = 2012,
           y = 1400, yend = 1600,
           curvature = -.3,
           color = "grey55",
           size = 1,
  ...
  theme(
    ...
    plot.title = element_marquee(family = "Roboto", size = 19),
    plot.subtitle = element_marquee(family = "Roboto", 
                                    margin = margin(b = 1.2, unit = "cm"),
                                    width = 1 # auto-wrapping
                                    ),
    strip.text = element_text(family = "Roboto", size = 12),
    strip.background = element_blank(),
    plot.background = element_rect(fill = "#fcefd0", colour = NA)
  )
```
...
ggsave(here::here("projects", "christmas-beetles", "plots", "counts_christmas-beetles_updated.png"),
       dpi = 320)
```
...
library(marquee)
title_text <- marquee_glue(
  "Species commonly misidentified as Christmas beetles (*Anoplognathus*)"
)
subtitle_text <- marquee_glue(
  "Christmas beetles are often mistaken for {#9a672a **Argentinian scarab beetles**} (an invasive species), {#979726 **Golden stag beetles**}, or various species of {#1E5A1E **June beetles**}. Despite this misidentification, every observation contributes valuable data about these species that would otherwise have had much less data recorded of them!"
)

# PLOT

ggplot(data = mis_id_joined) +
  geom_bar(aes(x = year, y = count, fill = name), stat = "identity", alpha = 0.9) + 
  scale_fill_manual(values = c(
    "Argentinian scarab beetle" = "#ba8240",
    "Golden stag beetle" = "#CBCB56",
    "June beetle (assorted species)" = "#1E5A1E"
  )) +
  labs(
    title = title_text,
    subtitle = subtitle_text,
...
ggsave(here::here("projects", "christmas-beetles", "plots", "counts_misID_updated.png"),
       dpi = 320)
```
