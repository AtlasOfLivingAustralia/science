...
# Define a function to create, save, and store the map for each dataset
create_and_save_map <- function(whale_data, month_name) {
  # Create the plot
  jan_map <- ggplot(data = world) +
    ...
    geom_point(data = whale_data, # whale data for the month
  ...
  ggsave(file_path, jan_map, width = 1524, height = 1524, units = "px", dpi = 320)
  
  # Store the plot as an object in the global map_plots list
  map_plots[[month_name]] <<- jan_map
}

# Apply the function for each month and dataset using walk2
purrr::walk2(whales_data, month_names, create_and_save_map)

```


```{r}
# Access and print each plot for all months
jan_plot <- map_plots[["jan"]]
print(jan_plot)

feb_plot <- map_plots[["feb"]]
print(feb_plot)

mar_plot <- map_plots[["mar"]]
print(mar_plot)

apr_plot <- map_plots[["apr"]]
print(apr_plot)

may_plot <- map_plots[["may"]]
print(may_plot)

jun_plot <- map_plots[["jun"]]
print(jun_plot)

jul_plot <- map_plots[["jul"]]
print(jul_plot)

aug_plot <- map_plots[["aug"]]
print(aug_plot)

sep_plot <- map_plots[["sep"]]
print(sep_plot)

oct_plot <- map_plots[["oct"]]
print(oct_plot)

nov_plot <- map_plots[["nov"]]
print(nov_plot)

dec_plot <- map_plots[["dec"]]
print(dec_plot)
...
ggsave("jan_map.png", jan_plot, width = 1524, height = 1024, units = "px", dpi = 320, bg = "transparent")
