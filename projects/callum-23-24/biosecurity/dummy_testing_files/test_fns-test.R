...
##### Libraries #####
{
  ...
  remotes::install_github("atlasoflivingaustralia/galah@new-names-test")
  library(galah)
  library(readr)
  library(dplyr)
  library(tidyr)
...
lists_path <- "biosecurity_alerts/data/edited_lists/"
list_suffix <- "_list"
synonym_delimiter <- ","
...
shapes_path <- "biosecurity_alerts/data/shapefiles/"
...
list_df <- collate_lists(lists_path)

species_list <- get_species_lists(list_df)

common_names <- assign_common_names(species_list)

species_records <- search_occurrences(species_list, common_names, 75, 0, 75, 0)
...
build_email(alerts_data = occ_full,
            ...
            output_path = NULL,
            email_list = email_list[1,], email_subject = "[TEST] ALA Biosecurity Alert",
