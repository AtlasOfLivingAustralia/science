...
##### get_species_lists() #####
lists_path <- #ADD_HERE path to "/biosecurity_alerts/data/edited_lists/" OR path to folder with dummy lists
list_suffix <- "_list"
synonym_delimiter <- ","
...
common_names
event_date_start <- 365
event_date_end <- 0
upload_date_start <- 365
upload_date_end <- 0
...
##### Load Shapefiles #####
load(
  #ADD_HERE path to "/koel/data/aus_territory.rda"
)
load(
  #ADD_HERE path to "koel/data/coastal_waters_shp.rda"
)

##### filter_occurrences() #####
species_records
shapes_path <- #ADD_HERE path to "biosecurity_alerts/data/shapefiles/" OR path to folder with dummy shapefiles
...
cache_path
template_path <- #ADD_HERE path to "biosecurity_alerts/email_template.Rmd"
output_path <- NULL
